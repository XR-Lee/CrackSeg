...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Visualize DeepLab 0-shot masks and SAM-refined crack masks."
    )
    parser.add_argument(
        "--data-root",
    ...
    parser.add_argument(
        ...
        type=Path,
        help="Path to save the comparison figure. Relative paths are resolved against this script's folder, not the current working directory.",
    )
    ...
    if not args.data_root.is_absolute():
        args.data_root = script_dir / args.data_root
    return args
...
def main():
    ...
    for row, stem in enumerate(stems):
        ...
        panels = [
            ...
            ("DeepLab 0-shot", overlay_mask(image, base_mask, color=(255, 0, 0))),
