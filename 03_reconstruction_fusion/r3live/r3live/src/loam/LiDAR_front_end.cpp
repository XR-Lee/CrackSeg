...
int main(int argc, char **argv)
{
    ...
    n.param< double >( "Lidar_front_end/min_dist", min_dist, 1);
    n.param< double >( "Lidar_front_end/max_dist", max_dist, 50);
    ...
    case HESAI32_M2X:
        printf("HESAI32_M2X\n");
        sub_points = n.subscribe("/Pandar/XT32_M2X/pandar", 1000, hesai32_handler, ros::TransportHints().tcpNoDelay());
        break;
...
{
    pcl::PointCloud<PointType> pl_processed;
    pcl::PointCloud<hesai_ros::Point> pl_pretf;
    pcl::PointCloud<hesai_ros::Point> pl_orig;
    // pcl::PointCloud<pcl::PointXYZI> pl_orig;
    pcl::fromROSMsg(*msg, pl_pretf);
    pcl::transformPointCloud(pl_pretf, pl_orig, lidar_rot);
    ...
    for (int i = 0; i < plsize; i++)
    {
        ...
        added_pt.normal_y = 0;
        added_pt.normal_z = 0;
        added_pt.curvature = (pl_orig.points[i].timestamp - scan_begin_time); // curvature unit: ms // cout<<added_pt.curvature<<endl;
        pl_processed.points.push_back(added_pt);
