...
Preprocess::Preprocess()
  :feature_enabled(0), lidar_type(AVIA), blind(0.01), max_range(10), point_filter_num(1)
{
  inf_bound = 10;
  N_SCANS   = 6;
  SCAN_RATE = 10;
...
void Preprocess::set(bool feat_en, int lid_type, double bld,double max_range, int pfilt_num)
{
...
void Preprocess::hesai_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
  ...
  for (int i = 0; i < plsize; i++)
  {
    ...
    {
      if(added_pt.x*added_pt.x+added_pt.y*added_pt.y+added_pt.z*added_pt.z > (blind * blind) && \
      added_pt.x*added_pt.x+added_pt.y*added_pt.y+added_pt.z*added_pt.z < (max_range * max_range))
      {
        pl_surf.points.push_back(added_pt);
      }
    }
