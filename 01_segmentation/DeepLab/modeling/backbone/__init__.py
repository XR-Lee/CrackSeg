...
def build_backbone(backbone, output_stride, BatchNorm, pretrained=True):
    ...
    if backbone == 'resnet_feat':
        return resnet_feat.ResNet101(output_stride, BatchNorm, pretrained=pretrained)
    elif backbone == 'xception':
        return xception.AlignedXception(output_stride, BatchNorm, pretrained=pretrained)
    elif backbone == 'xception_feat':
        return xception_feat.AlignedXception(output_stride, BatchNorm, pretrained=pretrained)
    elif backbone == 'drn':
